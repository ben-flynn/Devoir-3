...
public class Utils {
  ...
  /**
   * This method rotates array of size lines * columns referenced by
   * transformedBoard by 90 degree clockwise. For example, the 3*3 game
   *
   * 1 | 2 | 3 ---------- 4 | 5 | 6 ---------- 7 | 8 | 9
   *
   ...
   * @param transformedBoard Reference to a 1 dimentional array of size columns x
   *                         lines
   */

  public static void rotate(int lines, int columns, int[] transformedBoard) {
    // initialisation d'un tableau unidimentionnel temporaire
    int[] tempBoard = new int[(lines * columns)];
    // initialisation des variables compteurs de pas et de l'indice;
    int counter = 0;
    int a = 0;

    // Boucle externe controle la colonne
    for (int i = 0; i < columns; i++) {
      // Boucle interne controle la rangée
      for (int j = 0; j < lines; j++) {
        tempBoard[j + counter] = transformedBoard[((columns * (lines - 1)) + i) - (j * (columns))];

      }
      // incrémente le bond de rangée en rangée
      counter = counter + lines;
    }
    // Recopie les élement du tableau temporaire au transformedBoard
    for (int v : tempBoard) {
  ...
  /**
   * This method does an horizontal symmetry on array of size lines * columns
   * referenced by transformedBoard. For example, the 3*3 game
   *
   *
   * 1 | 2 | 3 ---------- 4 | 5 | 6 ---------- 7 | 8 | 9
   *
  ...
  public static void horizontalFlip(int lines, int columns, int[] transformedBoard) {
    ...
    int counter = 0;
    int a = 0;
    // Boucle externe controle la rangée
    for (int i = 0; i < lines; i++) {
      ...
      // incrémente le bond de colonne en colonne
  ...
  /**
   * This method does an vertical symmetry on array of size lines * columns
   * referenced by transformedBoard. For example, the 3*3 game
   *
   * 1 | 2 | 3 ---------- 4 | 5 | 6 ---------- 7 | 8 | 9
   *
  ...
  public static void verticalFlip(int lines, int columns, int[] transformedBoard) {
    ...
    for (int i = 0; i < lines; i++) {
      ...
      for (int j = columns - 1; j >= 0; j--) {
        // incrémente le bond de colonne en colonne après avoir affecter la valeur du
        // transformedBoard au tempBoard
